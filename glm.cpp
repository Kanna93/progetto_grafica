...
GLvoid
glmCenter(GLMmodel* model, GLfloat* center){
	center[0]= (model->bbmin[0]+model->bbmax[0])/2.0;
	center[1]= (model->bbmin[1]+model->bbmax[1])/2.0;
	center[2]= (model->bbmin[2]+model->bbmax[2])/2.0;
}
...
glmSecondPass(GLMmodel* model, FILE* file) 
{
    ...
    //printf("NUMERO GRUPPI %d ",model->numgroups);
    //printf("NUMERO GRUPPI %d ",model->numtexcoords);
...
void glmComputeBoundingBox(GLMmodel* model){
  if (model->numvertices==0) return;
  model->bbmin[0]=model->bbmax[0]=model->vertices[3];
  model->bbmin[1]=model->bbmax[1]=model->vertices[4];
  model->bbmin[2]=model->bbmax[2]=model->vertices[5];

  for ( int i=1;i<=(model->numvertices);i++){
    for (int k=0; k<3; k++) {
      if (model->bbmin[k]>model->vertices[3*i+k]) model->bbmin[k]=model->vertices[3*i+k];
      if (model->bbmax[k]<model->vertices[3*i+k]) model->bbmax[k]=model->vertices[3*i+k];
    }
  }
}
...
//            printf("glmVertexNormals(): vertex w/o a triangle %d\n", i);
